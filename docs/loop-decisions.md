...
| 25 | **因果敏感性 (oster_delta/evalue)** | oster R_max=min(1.3·R̃,1);evalue RR 转换 rare→OR、common→sqrt(OR)、连续→exp(.91d) Chinn,连续暴露>2级按中位二分 | 指定 outcome/treatment(exposure)/controls;调 r_max | ✅ 可配 oster `outcome`/`treatment`/`controls`/`r_max`;evalue `outcome`/`exposure`(或 treatment)/`controls`。(rosenbaum_bounds 撤,见 deferred-log) |
