...
- **A. Skill Ingestion（随时添加）**：`skills_inbox/` 投递 → 读 `SKILL.md` → 提炼（领域/方法/前提/产出）→ 登记进分析目录（catalog）→ 原件归档 `_processed/`。让"随时加 skill"成为一等公民。投递渠道有二：**手动**放入 inbox，或由「GitHub 采集器」（§7.4）**自动发现**后入列。
...
3. **前沿监视器（独立、可选，跑在 autonomous loop 上）**：定期追踪用户领域的新论文（arXiv/期刊），新方法自动进 catalog、相关新发现提醒用户。与现用的 `/loop` 自动机制天然契合。**本期不做，留待后续。**
