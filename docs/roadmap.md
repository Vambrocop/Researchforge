...
- [~] **贝叶斯/生存/缺失**:**贝叶斯回归·分层(2026-06-25 做了)** —— 原标"待 Stan/JAGS",**现代 PyMC NUTS 无需编译器**(Windows 无 g++ 也秒级采样),`bayesian_mcmc.py`:`bayesian_regression`/`bayesian_logistic_regression`/`bayesian_hierarchical`(变截距部分汇集),HDI/R-hat/ESS/ICC,可选+优雅降级。〔共轭先验贝叶斯 `bayesian_ab_test`/`bayesian_proportion`/`bayesian_poisson_rate` 仍在〕。贝叶斯扩展(2026-06-25):`bayesian_poisson_regression`(计数 GLM 自动 Poisson/NB)/`bayesian_robust_regression`(Student-t 抗离群)/`bayesian_model_comparison`(PSIS-LOO)。**MICE 多重插补(2026-06-25 做了)**:`missing_data` 族 `mice_imputation`(链式方程+Rubin 规则+FMI)+`missingness_diagnosis`(模式+MCAR 筛查)。**仍开**:竞争风险(已有 competing_risks)、时变协变量(已有 time_varying_cox)、贝叶斯 SEM(blavaan)。
- [x] **资源/可持续域(EWF nexus,2026-06-25)**:`resource` 族 5 法 —— `composite_index`(OECD/JRC 综合指数)+`input_output_analysis`(Leontief 投入产出)+**成本↔物理桥**:`cost_effectiveness_analysis`(CEA/ICER,$/单位)、`marginal_abatement_cost`(MACC 减排成本曲线)、`material_flow_analysis`(MFA 物质质量平衡)。配合 DEA/SFA/MCDA/网络/VAR-Granger + techno_economic(财务镜头),EWF 能源-水-食物 nexus 工具箱打通(财务×物理×权衡三镜头)。**收尾(2026-06-25)**:`footprint_analysis`(LCA 碳/水/物质足迹)、`life_cycle_cost`(LCC 全生命周期成本/EAC,extend techno)、MFA Sankey 流图。nexus 工具箱完整。
...
- [x] **运筹/博弈/市场/精算 4 领域波(2026-06-26)**:`operations_research`(EOQ/M-M-c/newsvendor)、`game_theory`(纳什均衡/Shapley)、`marketing`(RFM/CLV/购物篮 Apriori)、`actuarial`(生命表/链梯+Mack/损失分布 VaR-TVaR)。冷审 ZERO MUST(博弈对 Osborne-Rubinstein、Mack 对 Mack1993)。→274 法/45 族。
- [x] **因子库刷新到最新(2026-06-26)**:LCA 库 v0.2、DEFRA 2025 燃料、Ember 2026 电网(0.458)、真 WFN 水足迹、as-of 标注;诚实"分析不联网,库带版本可定期刷新"。
...
### 真用与长远双轴定盘（2026-07-12，Fable 主脑；Wave K/L 修完自动挡后）
**完整度盘点**（九维）：工程质量 90 · 数据管线 90 · 方法广度 85 · 诚实防护 85 · 自动挡 80 · 产品面 75 · 性能 70 · **分发/文档 60** · **自我进化 40**。内核已硬，短板全在外圈——路线按补短板排，不往 90 分维度堆力气。
- **真用轴（v1.1，进行中）**：U1 pip 打包+README 门面（装得上、60 秒上手——"给别人用"的最大缺口）· U2 真实数据端到端试用（sklearn/statsmodels 真数据集，第一次真数据摩擦日志）· web 浏览器实测（用户动作门）· 把用户自己的真实 CSV 跑起来（最真的试金石）。
- **收口轴（Wave M，与真用轴并行）**：M1 因果族去霸榜（无处理名列不得压 logistic/epi，P2 golden 转绿）· M2 P6 日期→is_timeseries 结构过检 · M3 deferred 尾巴批 · M4 三轮 dogfood=真·新 persona 新数据。**Gate M 过 → 宣布常见形态"选得对+聪明"**。
- **长远轴（v1.2+）**：v1.2 方法扩张恢复（DEA/SFA/QCA/BMA/FDA/面板计量补全，主战场五域优先，主公拍板方向）+ web 纵深 · v1.3 discover 真抓取深化（趋势→候选队列→自动评分，自我进化闭环=最低分维度）· v1.4 性能波（快速性 70=最弱健康维，Opus 剖析）+ C3 ColumnSemantics dataclass（等再被散落字段绊到才做）· v2.0 见下。
