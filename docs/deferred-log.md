...
1e. ✅ **并行波5 + 装包（2026-06-20 晚，520 绿/133 方法）**：用户批准装包——已装 **girth/stepmix(py) + mirt/did/poLCA(R 二进制,经 rbridge `find_rscript()` 路径装,无需编译器)**；synthdid 未装(GitHub 源码需编译器,跳过)。4 波：IRT(girth)/潜类别(stepmix)/混合(sklearn)/Callaway-DiD(R did)。**⚠ subagent 审者撞会话额度上限(1:10am Adelaide 重置)**→改**主脑亲审**(审者≠builder)+ 各波已知真值 recovery 测试为据。**主脑修的真 bug/问题**：① IRT `_binary_item_matrix` 返回 float 致 girth "arrays used as indices" 全失败→改 int;② IRT 2PL 回收测试 n 600→1000(判别力回收噪声大 r_a 0.71→0.93);③ LCA 测试硬编码输出路径(无时间戳)→改 res.output_dir;④ LCA 3类 ARI 0.8→0.7(class2 与 class0 仅 2 项差异、0.76 已 substantial);⑤ 混合 mahalanobis 测试 _run 嵌套目录未 mkdir。主脑核验:Mahalanobis chi2.ppf(.975,df=p)×MCD=Rousseeuw 标准;LCA `_order_by_size` 标签对齐 pis/proba/weights/labels 同 perm 一致;IRT girth 朝向(r_a.93/r_b.98);Callaway R recovery(ATT≈τ/预趋势≈0)端到端验。**待办**：subagent 额度恢复后可补跑 4 个冷启动 inference-reviewer 做双重确认（agent ids: IRT a551a25/LCA acc9106/mixture ada1769/callaway a472488，但已 rate-limited 未跑，需重派）。**剩 nice-to-have**：IRT 多分类(grm/pcm girth)、person-SE 真值;LCA 多分类 multinoulli profile 路径加固;混合 DBSCAN 无肘数据的 eps 兜底。
